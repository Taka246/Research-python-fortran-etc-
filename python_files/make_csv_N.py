...
data_N = pd.DataFrame([dimer_id, time, distance_m, distance_g, distance_w, theta1, theta2, theta_n, theta_w]).T
data_N.columns = ['Dimer ID', 'Time (ps)', 'Mean Distance', 'Centroid Distance', 'Distance of Wing Roots', 'Angle of Wing1', 'Angle of Wing2', 'Theta_n', 'Theta_w']

df_PTCDI_N = data_N.apply(pd.to_numeric, errors='ignore')
df_PTCDI_N.to_csv('N_PTCDI.csv')
