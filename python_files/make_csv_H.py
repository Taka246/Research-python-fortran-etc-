...
def H(xyz):
    ...
    wing_right1 = unit(N_vec.iloc[1:2,:3] - N_vec.iloc[0,:3].values) #H_dimer '中心'ー'羽'　47-55,99-74
    wing_left1 = unit(N_vec.iloc[2:3,:3] - N_vec.iloc[3,:3].values)
    wing_right2 = unit(N_vec.iloc[1:2,3:] - N_vec.iloc[0,3:].values)
    wing_left2 = unit(N_vec.iloc[2:3,3:] - N_vec.iloc[3,3:].values)

    wing1 = (N_vec.iloc[0,:3].values + N_vec.iloc[3,:3].values)/2 #羽根元間距離byモノマー
    wing2 = (N_vec.iloc[0,3:].values + N_vec.iloc[3,3:].values)/2
...
time = []
distance_w = []
for i,model in enumerate(H_models):
...
data_H = pd.DataFrame([dimer_id, time, distance_m, distance_g, distance_w, theta1, theta2, theta_n, theta_w]).T
data_H.columns = ['Dimer ID', 'Time (ps)', 'Mean Distance', 'Centroid Distance', 'Distance of Wing Roots', 'Angle of Wing1', 'Angle of Wing2', 'Theta_n', 'Theta_w']

df_PTCDI_H = data_H.apply(pd.to_numeric, errors='ignore')
df_PTCDI_H.to_csv('H_PTCDI.csv')
